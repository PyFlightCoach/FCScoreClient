...
export const downgrade_info = (result: Result) => {
...
export const criteriaInfo = (criteria: Criteria, result: Result) => {
	const makeRad = result.measurement.unit.includes('rad');
	console.log(makeRad);
	const scale = makeRad ? 180 / Math.PI : 1;
	const x = makeRad
		? linspace(0.0, 120.0, 20.0)
		: linspace(0.0, Math.max(Math.max(...result.errors), -Math.min(...result.errors)) * 2, 20.0);
	const y = x.map((v) =>
		Math.min(
			criteria.lookup.factor * Math.pow(v / scale, criteria.lookup.exponent),
			criteria.lookup.limit
		)
	);

	return [
		{ type: 'scatter', x, y, line: { color: 'black' }, showlegend: false },
		{
			type: 'scatter',
