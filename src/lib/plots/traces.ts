...
export const criteriaInfo = (criteria: Record<string, any>) => {
  const makeRad = ['ContAbs', 'Single'].includes(criteria.kind);
  console.log(makeRad);
  const scale = makeRad ? 180/Math.PI : 1;
  const x = makeRad ? linspace(0.0, 90.0, 20.0) : linspace(0.0, 5.0, 20.0);
