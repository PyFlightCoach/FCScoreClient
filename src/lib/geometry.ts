...
export class State {
	...
	constructor(
		...
		readonly dv: number,
		readonly dw: number,
		manoeuvre: string | undefined = undefined,
	...
	static build(
		pos: Point,
		att: Quaternion,
		vel: Point = new Point(0, 0, 0),
		rvel: Point = new Point(0, 0, 0),
	...
	direction_str() {
		return this.direction() > 0 ? 'LefttoRight' : 'RighttoLeft';
	}
}
...
export class States {
	...
	static parse(data: Record<string, any>[] | Record<string, Record<string, any[]>>): States {
		return new States(
			(data.hasOwnProperty('data') ? data.data : data).map((st) => State.parse(st))
		);
	}
