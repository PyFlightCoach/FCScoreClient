...
<script lang="ts">
  ...
  import {flightdata, mname, difficulty, truncate} from '$lib/stores';
  ...
  let colours = redsColors;
  $: colours = [yellColors, tealsColrs, redsColors][$difficulty-1];
  $: man = flightdata.mans[manname];
  $: busy = $man.busy;
  $: scores = $man.get_first_matching_score({difficulty: $difficulty, truncate: $truncate});
  $: intra = scores===null ? 0 : scores.intra;
  $: inter = scores===null ? 0 :  scores.inter;
  $: position = scores===null ? 0 :  scores.positioning;


  $: score = scores===null ? 0 : scores.total; 
...
{#if !(scores===null)}
  ...
  <button style:background-color={colscale(position, 6, colours)} data-sveltekit-preload-data="tap" on:click={()=>activate_man(manname, 'positioning')}>{position.toFixed(2)}</button>
{:else}
  <div>-</div>
  <div>-</div>
  <div>-</div>
